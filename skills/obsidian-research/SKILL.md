...
Use `obsidian_retrieve` for Obsidian vault retrieval, including `mode: "relationships"` for one explicit safe Markdown note path. Use `obsidian_plan` for read-only bounded non-destructive previews when the user asks to plan. Use `obsidian_validate` for read-only workflow-neutral Markdown validation. Use `obsidian_config` only when the user gives a vault folder path or asks to forget/check the remembered vault. For explicit permanent delete/full replace requests, use `obsidian_destroy`. For normal mutation requests, call `obsidian_write`, `obsidian_edit`, or `obsidian_manage` directly; the extension previews internally, asks the human for approval in Pi, and commits only if approved.
...
9. If setup is missing and the user provides an Obsidian vault folder path, call `obsidian_config` with `operation: "set_vault"`. This is the only normal persistent setting.
...
- Never use `obsidian_validate` for templates, template variables, path generation, automatic folder creation, backlinks, relationship summaries, transaction previews, link rewriting, broad vault access, shell/network calls, UI open behavior, or arbitrary CLI execution.
