...
description: Retrieve compact Obsidian vault context for research tasks through the read-only obsidian_retrieve tool. Safe writing is handled separately by obsidian_write, and safe structured edits are handled separately by obsidian_edit.
---

# Obsidian Research

Use `obsidian_retrieve` for all Obsidian vault retrieval. Use `obsidian_write` only for explicit create/append requests, and use `obsidian_edit` only for explicit structured edits to existing Markdown notes.
...
- Never use any Obsidian tool for full-note overwrite, delete, rename, move, UI open, shell execution, network calls, broad scanning, regex/fuzzy/semantic replacement, inferred target text, or arbitrary CLI commands.
