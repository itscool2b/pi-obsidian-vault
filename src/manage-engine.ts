...
import { buildMovePreview, buildRestorePreview, buildTrashPreview, makeManageError, makeManageOutput, normalizeManageOperation } from "./manage-guidance.js";
...
export async function obsidianManage(request: ObsidianManageRequest, options: ObsidianManageOptions = {}): Promise<ObsidianManageOutput> {
  ...
  if (!request.operation?.trim()) {
    return makeManageOutput({
      ...
      dryRun,
      message: "obsidian_manage requires operation=move_note, operation=trash_note, or operation=restore_note.",
      error: makeManageError("MISSING_OPERATION", "validation", "Provide operation=move_note, operation=trash_note, or operation=restore_note."),
    });
  }
  ...
  if (!op.operation) {
    ...
    return makeManageOutput({
      ...
      dryRun,
      message: "obsidian_manage supports only move_note, trash_note, and restore_note; the requested operation was refused.",
      error: makeManageError(code, "safety", "Use only operation=move_note, operation=trash_note, or operation=restore_note. Folder moves, permanent delete, recursive delete, wildcard/bulk operations, overwrite, copy, link rewrite, UI, shell, network, scan, discovery, and arbitrary command operations are not supported."),
