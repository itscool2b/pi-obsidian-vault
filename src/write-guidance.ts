...
export function normalizeOperation(value: string | undefined): { operation?: ObsidianWriteOperation | undefined; requested?: string | undefined; forbidden: boolean } {
  ...
  const forbidden = FORBIDDEN_OPERATIONS.has(requested) || /overwrite|replace|truncate|prepend|delete|remove|unlink|erase|discard|trash|restore|recycle|rename|move|open|launch|shell|bash|exec|command|curl|fetch|network|scan|discover|rmdir/.test(requested);
