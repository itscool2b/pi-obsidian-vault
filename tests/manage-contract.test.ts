...
describe("obsidian_manage contract", () => {
  it("registers obsidian_manage separately with strict top-level fields", () => {
    ...
    expect(surfaceText).toMatch(/folders/i);
  ...
  it("returns structured copy_note preview and committed success through the registered tool", async () => {
    await withTempVault(async (vaultRoot) => {
      await seedNote(vaultRoot, "Projects/Plan.md", "# Plan\n");
      await seedFolder(vaultRoot, "Archive");
      const tool = registerManageTool(vaultRoot);

      const preview = await tool.execute("id", { operation: "copy_note", fromPath: "Projects/Plan.md", toPath: "Archive/Plan.md" });
      expect(preview.details).toMatchObject({ tool: "obsidian_manage", status: "preview", operation: "copy_note", fromPath: "Projects/Plan.md", toPath: "Archive/Plan.md", dryRun: true, committed: false, preview: { wouldCopy: true, wouldOverwrite: false, wouldCreateParent: false, wouldMoveSource: false, wouldDeleteSource: false, wouldRewriteLinks: false } });
  ...
  it("returns validation, safety, not_found, conflict, and setup outcomes", async () => {
    await withTempVault(async (vaultRoot) => {
      ...
      expect(await obsidianManage({ operation: "copy_note", toPath: "Archive/Plan.md" }, { vaultRoot })).toMatchObject({ status: "validation_error", error: { code: "MISSING_FROM_PATH" } });
      ...
      expect(await obsidianManage({ operation: "copy_note", fromPath: "Projects/Plan.txt", toPath: "Archive/Plan.md" }, { vaultRoot })).toMatchObject({ status: "validation_error", error: { code: "SOURCE_NOT_MARKDOWN" } });
      expect(await obsidianManage({ operation: "copy_note", fromPath: "Projects/Plan.md", toPath: "Archive/Plan.txt" }, { vaultRoot })).toMatchObject({ status: "validation_error", error: { code: "TARGET_NOT_MARKDOWN" } });
      expect(await obsidianManage({ operation: "copy_note", fromPath: "Projects/Plan.md", toPath: "@Projects/Plan.md" }, { vaultRoot })).toMatchObject({ status: "validation_error", error: { code: "SAME_PATH" } });
      ...
      expect(await obsidianManage({ operation: "copy_note", fromPath: "Projects/Plan.md", toPath: "Archive/Plan.md", dryRun: false }, { vaultRoot })).toMatchObject({ status: "conflict", error: { code: "TARGET_EXISTS" } });
    });
