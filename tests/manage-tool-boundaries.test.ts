...
describe("obsidian_manage tool boundaries", () => {
  it("keeps retrieval read-only for move, rename, trash, restore, copy, and delete intent", async () => {
    const backend = seededFakeCli();
    const result = await obsidianRetrieve(backend, { query: "move rename trash restore copy or delete a note in obsidian", mode: "search" });
  ...
  it("keeps obsidian_write and obsidian_edit from accepting move_note, trash_note, restore_note, or copy_note", async () => {
    await withTempVault(async (vaultRoot) => {
      await seedNote(vaultRoot, "Notes/Existing.md", "# Existing\n");
      for (const operation of ["move_note", "trash_note", "restore_note", "copy_note", "trash", "restore", "copy", "delete"]) {
  ...
  it("refuses unsupported and forbidden management actions without side effects", async () => {
    await withTempVault(async (vaultRoot) => {
      ...
      for (const operation of ["rename", "move", "copy", "duplicate", "clone", "delete", "remove", "unlink", "trash", "restore", "recover", "untrash", "recycle", "recursive_delete", "bulk_delete", "wildcard_delete", "recursive_restore", "bulk_restore", "wildcard_restore", "recursive_copy", "bulk_copy", "wildcard_copy", "overwrite", "rewrite_links", "move_folder", "delete_folder", "restore_folder", "copy_folder", "create", "append", "replace_exact_text", "open", "shell", "network", "scan", "discover", "command"]) {
