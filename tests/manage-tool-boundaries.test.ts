...
describe("obsidian_manage tool boundaries", () => {
  it("keeps retrieval read-only for move, rename, trash, and delete intent", async () => {
    const backend = seededFakeCli();
    const result = await obsidianRetrieve(backend, { query: "move rename trash or delete a note in obsidian", mode: "search" });
    expect(result.warnings.join("\n")).toMatch(/read-only/i);
    expect(backend.calls.map((call) => call.method)).not.toEqual(expect.arrayContaining(["write", "append", "edit", "delete", "trash", "rename", "move", "open"]));
  });

  it("keeps obsidian_write and obsidian_edit from accepting move_note or trash_note", async () => {
    await withTempVault(async (vaultRoot) => {
      await seedNote(vaultRoot, "Notes/Existing.md", "# Existing\n");
      for (const operation of ["move_note", "trash_note", "trash", "delete"]) {
  ...
  it("refuses unsupported and forbidden management actions without side effects", async () => {
    await withTempVault(async (vaultRoot) => {
      ...
      for (const operation of ["rename", "move", "copy", "delete", "remove", "unlink", "trash", "recycle", "recursive_delete", "bulk_delete", "wildcard_delete", "overwrite", "rewrite_links", "move_folder", "delete_folder", "create", "append", "replace_exact_text", "open", "shell", "network", "scan", "discover", "command"]) {
